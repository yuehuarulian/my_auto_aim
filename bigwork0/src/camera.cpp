...
namespace auto_aim
{
    ...
    cv::Mat MVCamera::getFrame()
    {
        ...
        if (CameraGetImageBuffer(hCamera, &sFrameInfo, &pbyBuffer, 1000) == CAMERA_STATUS_SUCCESS)
        {
            ...
            auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(endTime - startTime); // ms
            durationInSeconds = std::chrono::duration_cast<std::chrono::duration<double>>(duration).count();//s
            ////
            return matImage;
        }
